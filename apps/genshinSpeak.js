...
export class GenshinSpeak extends plugin {
  ...
  async cyy(e) {
    ...
    if ((e.msg == '原神猜语') | ((e.msg == '猜语音') & (ks == 0))) {
      ...
      let jsdz =
        ml +
        '/plugins/earth-k-plugin/resources/html/GenshinSpeak/' +
        'Characters/' +
        wenj2[i] +
        '.txt';
      let wb = '';
  ...
  async tingyy(e) {
    ...
    //兼容小飞插件 #(高清)语音x 指令
    if (e.msg.startsWith('#高清语音') || e.msg.startsWith('#语音')) return;
    ...
    let jsdz =
      ml + '/plugins/earth-k-plugin/resources/html/GenshinSpeak/' + 'Characters/' + name4 + '.txt';
    ...
    for (let n = 0; n < 17; n++) {
      for (let i = 0; i < res2.data.list[0].children[n].list.length - 1; i++) {
        id = id + 1;
        syid.push({
          fenlei: res2.data.list[0].children[n].name,
          id: id,
          content_id: res2.data.list[0].children[n].list[i].content_id,
          title: res2.data.list[0].children[n].list[i].title,
        });
      }
    }
