...
export class ShareMusic extends plugin {
  ...
  async wybofang(e) {
    ...
    let ckxx = YAML.parse(fs.readFileSync('./plugins/earth-k-plugin/config/config.yaml', 'utf8'));
    wyck = ckxx.wyck;
    ...
    await SendMusicShare(e, {
      source: 'netease',
      name: data2.result.songs[0].name,
      artist: data2.result.songs[0].artists[0].name,
      pic: data2.result.songs[0].artists[0].img1v1Url,
  ...
  async bofang(e) {
    ...
    await SendMusicShare(e, {
      name: data2.data.song,
      artist: data2.data.singer,
      pic: data2.data.picture,
      link: 'https://y.qq.com/n/ryqq/songDetail/',
      url: data2.data.music,
    });
  }
  async txck(e) {
    let ckxx = YAML.parse(fs.readFileSync('./plugins/earth-k-plugin/config/config.yaml', 'utf8'));
    ...
    try {
      ...
      if (Number(id) > 0) {
        if (qq) {
          ...
          await SendMusicShare(e, {
            name: data[id - 1].song,
            artist: data[id - 1].singers,
            pic: data[id - 1].picture,
            link: 'https://y.qq.com/n/ryqq/songDetail/',
            url: data2.data.music,
          });
      ...
      if ((id != '') & isWangYiyun) {
        ...
        let options = {
          ...
          headers: {
            ...
            'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 12; MI Build/SKQ1.211230.001)',
            Cookie:
              'versioncode=8008070; os=android; channel=xiaomi; ;appver=8.8.70; ' +
              'MUSIC_U=' +
              wyck,
          },
          body: `ids=${JSON.stringify([ids])}&level=standard&encodeType=mp3`,
        ...
        await SendMusicShare(e, {
          source: 'netease',
          name: songList[id - 1].name,
          artist: songList[id - 1].artists[0].name,
          pic: songList[id - 1].artists[0].img1v1Url,
          link: 'https://music.163.com/#/song?id=' + ids,
          url: url,
        });
